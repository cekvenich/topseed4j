...
public class Parser {
	...
	public Parser(String filename, String basePath, TemplateLoader templateLoader, ExpressionHandler expressionHandler)
			throws IOException {
	...
	public Parser(String src, String filename, String basePath, TemplateLoader templateLoader,
			ExpressionHandler expressionHandler) throws IOException {
		this.filename = filename;
		this.basePath = basePath;
		this.templateLoader = templateLoader;
		this.expressionHandler = expressionHandler;
	...
	private Node parseMixinBlock() {
		...
		if (this.inMixin == 0) {
			throw new JadeParserException(filename, lexer.getLineno(), templateLoader,
	...
	private Parser createParser(String templateName) {
		...
		try {
			return new Parser(pathHelper.resolvePath(filename, templateName, basePath, templateLoader.getExtension()),
					basePath, templateLoader, expressionHandler);
		} catch (IOException e) {
			throw new JadeParserException(filename, lexer.getLineno(), templateLoader,
	...
	private Node tag(AttrsNode tagNode) {
		...
		while (true) {
			...
			if (incomingToken instanceof CssId) {
				...
				if (seenAttrs) {
					throw new JadeParserException(filename, line(), templateLoader,
							this.filename + ", line " + this.peek().getLineNumber()
									+ ":\nYou should not have jade tags with multiple attributes.");
					// console.warn(this.filename + ', line ' + this.peek().line + ':\nYou should
					// not have jade tags with multiple attributes.');
				}
				seenAttrs = true;
	...
	private Node[] parseInlineTagsInText(String str) {
		...
		if (matcher.find(0) && matcher.groupCount() > 1) {
			if (matcher.group(1) != null) { // escape
				...
				Parser inner = null;
				try {
					inner = new Parser(range.getSrc(), this.filename, this.basePath, this.templateLoader,
							this.expressionHandler); // Need to be reviewed
				} catch (IOException e) {
	...
	private Node parseCase() {
		...
		while (!(peek() instanceof Outdent)) {
			if (peek() instanceof Comment) {
				...
				throw new JadeParserException(filename, lexer.getLineno(), templateLoader,
						"Unexpected token \"" + this.peek() + "\", expected \"when\", \"default\" or \"newline\"");
			}
		}
		expect(Outdent.class);
		node.setBlock(block);
	...
	private Token expect(Class expectedTokenClass) {
		...
		if (t.getClass().equals(expectedTokenClass)) {
			...
			throw new JadeParserException(filename, lexer.getLineno(), templateLoader, expectedTokenClass,
					t.getClass());
		}
	}
