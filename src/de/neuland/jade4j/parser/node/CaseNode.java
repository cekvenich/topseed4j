...
public class CaseNode extends Node {
	...
	public void execute(IndentWriter writer, JadeModel model, JadeTemplate template) throws JadeCompilerException {
		try {
			...
			for (Node when : block.getNodes()) {
				if (skip || "default".equals(when.getValue())
						|| checkCondition(model, when, template.getExpressionHandler())) {
					skip = false;
	...
	private Boolean checkCondition(JadeModel model, Node caseConditionNode, ExpressionHandler expressionHandler)
