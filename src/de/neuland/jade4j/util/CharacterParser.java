...
public class CharacterParser {
	...
	// while (state.roundDepth >= 0 && state.curlyDepth >= 0 && state.squareDepth >=
	// 0) {
	// if (index >= src.length) {
	// throw new Error('The end of the string was reached with no closing bracket
	// found.');
	// }
	// exports.parseChar(src[index++], state);
	// }
	...
	// if (prop === undefined) {
	// throw new Error('Bracket specified (' + JSON.stringify(bracket) + ') is not
	// one of ")", "]", or "}";');
	// }
	// while (state[prop] >= 0) {
	// if (index >= src.length) {
	...
	public Match parseMaxBracket(String src, char bracket, Options options) throws SyntaxError {
		...
		if (bracket != ')' && bracket != '}' && bracket != ']') {
			throw new SyntaxError(
					"Bracket specified (" + String.valueOf(bracket) + ") is not one of \")\", \"]\", or \"}\"");
		}
		while (getStateProp(state, bracket) >= 0) {
			if (index >= src.length()) {
				throw new SyntaxError(
						"The end of the string was reached with no closing bracket \"" + bracket + "\" found.");
			}
			this.parseChar(src.charAt(index++), state);
		}
		int end = index - 1;
	...
	// (!includeLineComment && state.lineComment) || !startsWith(src, delimiter,
	// index)) {
	// exports.parseChar(src[index++], state);
	// }
	// var end = index;
	...
	// if (character.length !== 1) throw new Error('Character must be a string of
	// length 1');
	// state = state || exports.defaultState();
	// state.src = state.src || '';
	// state.src += character;
	...
	// state.squareDepth--;
	// }
	// if (!state.blockComment && !state.lineComment && !wasComment) state.history =
	...
	public static class State {
		...
		public boolean isNesting() {
			return this.isString() || this.isComment() || this.regexp || this.roundDepth > 0 || this.curlyDepth > 0
					|| this.squareDepth > 0;
		}
	...
	public boolean isKeyword(String id) {
		return ("if".equals(id)) || ("in".equals(id)) || ("do".equals(id)) || ("var".equals(id)) || ("for".equals(id))
				|| ("new".equals(id)) || ("try".equals(id)) || ("let".equals(id)) || ("this".equals(id))
				|| ("else".equals(id)) || ("case".equals(id)) || ("void".equals(id)) || ("with".equals(id))
				|| ("enum".equals(id)) || ("while".equals(id)) || ("break".equals(id)) || ("catch".equals(id))
				|| ("throw".equals(id)) || ("const".equals(id)) || ("yield".equals(id)) || ("class".equals(id))
				|| ("super".equals(id)) || ("return".equals(id)) || ("typeof".equals(id)) || ("delete".equals(id))
				|| ("switch".equals(id)) || ("export".equals(id)) || ("import".equals(id)) || ("default".equals(id))
				|| ("finally".equals(id)) || ("extends".equals(id)) || ("function".equals(id))
				|| ("continue".equals(id)) || ("debugger".equals(id)) || ("package".equals(id))
				|| ("private".equals(id)) || ("interface".equals(id)) || ("instanceof".equals(id))
				|| ("implements".equals(id)) || ("protected".equals(id)) || ("public".equals(id))
				|| ("static".equals(id));
	}
	...
	//
	// //unless its an `if`, `while`, `for` or `with` it's a divide, so we assume
	// it's a divide
	...
	// //if the last thing was a keyword then it must be a regexp (e.g. `typeof
	// /foo/`)
	// if (/^\w+\b/.test(history) &&
	// isKeyword(/^\w+\b/.exec(history)[0].split('').reverse().join(''))) return
	// true;
	//
	// return false;
	// }
	public boolean isRegexp(String history) {
		...
		// unless its an `if`, `while`, `for` or `with` it's a divide, so we assume it's
		// a divide
		if (history.charAt(0) == ')')
			return false;
